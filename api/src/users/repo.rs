...
// CITEXT columns are selected via an explicit `::TEXT` cast so sqlx maps them
// to `String` without needing a custom type — the cast is free at runtime
// (CITEXT is binary-compatible with TEXT) and makes the macro introspection
// happy on every Postgres version.
...
/// untouched. Caller is responsible for already-hashing the password and for
/// authorising the role change.
pub async fn update(
    pool: &PgPool,
    id: Uuid,
    email: Option<&str>,
